...
begin
  ...
  InputQuery('Добавление социального статуса', 'Введенеи наименование', name);
  SocStatus.name := name;
  SocStatus.New;
...
begin
  ...
  name := InputBox('Редактирование социального статуса',
    'Введенеи новое наименование', dm.QSocStatusBrowsename.AsString);
  SocStatus.name := name;
  SocStatus.ChangeAtr;
  dm.RefTable(dm.QSocStatusBrowse);
end;
...
begin
  if MessageBox(handle, 'Удалить социальный статус', 'Удаление записи',
