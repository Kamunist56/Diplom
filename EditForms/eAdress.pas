...
var
  s: String;
begin
  if InputQuery('Добавление записи', 'Укажите наименование', s) then
  begin
...
begin
  Table_ := TSmallTables.Create(TableName);
  Table_.GetAtr(id);
  Table_.name := InputBox('Редактирование записи',
    'Введите ' + #13 + 'новое наименование', Table_.name);
  Table_.ChangeAtr;
  Table_.Destroy;
  Query.Close;
  Query.Open;
...
procedure TAdressEdit.DelRecord(TableName: String; Query: TADoQuery;
  id: integer);
begin
  if MessageBox(handle, 'Удалить запись?', 'Удаление записи',
